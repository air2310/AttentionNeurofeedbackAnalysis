...
if (options.traintype == 1) {
  subnumbers = c(1, 2, 4, 8, 9, 18, 21 )
} else {
  subnumbers = c(10, 11, 19, 22)
}
...
tmp = RT_ALL
tmp[ACCURACY_ALL==responseopts.incorrect & ACCURACY_ALL==responseopts.falsealarm  ,] = NaN
...
# Save Results
RESULTS = list(RT=RTplot, ACC =ACCPLOT2)
direct.results = paste(direct.resultsRoot, "Train", strings.traintype[options.traintype], "/", bids.substring, sep = "")
...
# savefilename = paste(direct.results, "/", bids.substring, "Behave_pre_post_results.rds", sep = "")
# RESULTS2=readRDS(file = savefilename)
