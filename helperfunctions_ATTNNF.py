...
class SetupMetaData:
    ...
    def __init__(self, attntrained):
        ...
        if (self.attntrained == 0): # Space
            ...
            self.subsIDXcollate = np.array(([10, 11, 19, 22, 28, 29, 43, 45]))
            self.subsIDXall = np.array(([10, 11, 19, 22, 28, 29, 43, 45]))

        else: # Feature
            self.subsIDX = np.array(([ 41 ])) # 1, 2,
            self.subsIDXcollate = np.array(([1, 2, 4, 8, 9, 18, 21, 23, 41]))
            self.subsIDXall = np.array(([1, 2, 4, 8, 9, 18, 21, 23, 41 ]))
